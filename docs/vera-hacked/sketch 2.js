...
// "L",
// ]
let parts = ['L A B R A C A D A B R A L A B R A C A D A B R A L A B R A C A D A B R A']
let threshold = 50
let rows = 30
let margin = 30
let repeat = 50
...
function drawPoster() {
	...
	for(let i = 0; i < boxes.length; i++) {
		...
		let b = parts[boxes[i][4]%parts.length].length
		if(lineToShow > 1 ) {
			b = parts[lineToShow].length
		}
		b = constrain(b, 0, ceil( (boxes[i][2]-textWidth("i"))/textWidth("i")) )
...
function updateGraphics() {
	...
	pg.normalMaterial();
	// pg.lights()
	pg.rotateY(frameCount/40*PI+HALF_PI)
	pg.rotateZ(frameCount/40*PI)
	// // pg.rotateX(frameCount*0.025)
	// pg.rotateX(frameCount/40*PI+HALF_PI)
	// pg.rotateX(HALF_PI)
